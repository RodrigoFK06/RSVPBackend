...
# from app.models.session import ReadingSession # Removed import
...
async def create_quiz(
    ...
    try:
        ...
        ai_params = await assess_text_parameters(updated_rsvp_session.text) # Use text from updated session
        ...
        await updated_rsvp_session.save() # Save RsvpSession with quiz_questions and AI params
    ...
    except FileNotFoundError: # This would be if RsvpSession.get inside service fails, though unlikely if it passed above
...
async def validate_quiz_answers(
    ...
    try:
        ...
        # Could add:
        # target_session = await RsvpSession.get(validation_input.rsvp_session_id) # Changed to RsvpSession
        # if not target_session:
        #     raise HTTPException(status_code=404, detail="RsvpSession not found") # Changed
        # if target_session.user_id != str(current_user.id):
        #     raise HTTPException(status_code=403, detail="User not authorized to validate quiz for this RsvpSession") # Changed
        ...
        return QuizValidateOutput(
            rsvp_session_id=quiz_attempt_doc.rsvp_session_id, # This field in QuizAttempt now stores RsvpSession ID
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="RsvpSession not found for validation.") # Changed
    except ValueError as ve: # Catch specific errors like "No quiz questions"
        logger.warning(f"Validation error for RsvpSession {validation_input.rsvp_session_id} by user {current_user.email}: {ve}") # Changed
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(ve))
    except Exception as e:
        logger.error(f"Error validating quiz for RsvpSession {validation_input.rsvp_session_id} by user {current_user.email}: {e}", exc_info=True) # Changed
